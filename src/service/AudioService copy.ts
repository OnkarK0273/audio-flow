...
export class AudioService {
  ...
  async startSession(connectConfig: ConnectConfig) {
    ...
    try {
      ...
      const modelToUse = connectConfig.model || DEFAULT_MODEL;
      ...
      // Connect to the Gemini Live API session
      ...
      // Initialize browser AudioContext at 16kHz for linear PCM stream
      ...
      // Add the mic-processor worklet module
      ...
      // Handle raw PCM frames from the audio worklet
      this.workletNode.port.onmessage = (event: MessageEvent<Float32Array>) => {
        const float32Data = event.data;
        if (!float32Data || this.isMuted) return;

        // Calculate RMS audio level (0-100) for real-time waveform visualization
        if (this.callbacks.onAudioLevel) {
          const level = calculateAudioLevel(float32Data);
          this.callbacks.onAudioLevel(level);
        }

        // Convert Float32 to 16-bit PCM Blob and stream to active session
  ...
  private handleMessage(message: LiveServerMessage) {
    ...
    // 1. Check for real-time inputTranscription
    ...
    // 2. Low-latency interim input transcription (live words while speaking)
    ...
    // 3. Fallback: If model emits parts directly
    ...
    // 4. Turn completion: commit the final recognized transcription
